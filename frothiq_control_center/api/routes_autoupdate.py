...
_MC3_BACKEND_SRC = Path("/home/frothiq/frothiq-control-center/frothiq_control_center")
_MC3_BACKEND_MAIN = Path("/home/frothiq/frothiq-control-center/main.py")
_MC3_BACKEND_DEPLOY = Path("/usr/lib/frothiq-control-center/backend/frothiq_control_center")
...
def _read_mc3_backend_version() -> str:
    init_src = _PLUGIN_SRC.parent / "frothiq-control-center" / "frothiq_control_center" / "__init__.py"
    try:
...
def _deploy_mc3_backend() -> dict:
    ...
    # Sync frothiq_control_center package
    _, err, rc = _run(
        ["sudo", "rsync", "-a", "--delete",
    ...
    if rc != 0:
        errors.append(f"rsync frothiq_control_center: {err.strip()}")
    else:
        steps.append("Synced frothiq_control_center package")
